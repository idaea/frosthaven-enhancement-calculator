...
export class EnhancementCalculator extends Component<Props, any> {
	...
	calculateCost() {
		...
		if (this.state.stickerType === "playerPlus1") {
			if (this.state.playerPlusOneAbility) {
				cost +=
					playerPlusOneAbilityLines[this.state.playerPlusOneAbility].cost;
			} else {
			...
			if (this.state.summonPlusOneAbility) {
				cost +=
					summonPlusOneAbilityLines[this.state.summonPlusOneAbility].cost;
			} else {
				// cannot yet calculate
				return 0;
			}
	...
	render() {
		...
		for (let stickerType in stickerTypes) {
			if (stickerTypes.hasOwnProperty(stickerType)) {
				...
				if (stickerType === "attackHex") {
					...
					icons = (
						<GloomhavenIcon icon="generalPlusOne" width={iconWidth} />
					);
				}
		...
		for (let playerPlusOneAbilityLine in playerPlusOneAbilityLines) {
			if (
				playerPlusOneAbilityLines.hasOwnProperty(playerPlusOneAbilityLine)
			) {
				...
				playerPlusOneAbilityColumns.push(
					...
					>
						<Button
							...
							block
							onClick={() =>
								this.playerPlusOneAbilityClick(playerPlusOneAbilityLine)
							}
							className={
		...
		for (let summonPlusOneAbilityLine in summonPlusOneAbilityLines) {
			if (
				summonPlusOneAbilityLines.hasOwnProperty(summonPlusOneAbilityLine)
			) {
				...
				summonPlusOneAbilityColumns.push(
					...
					>
						<Button
							...
							block
							onClick={() =>
								this.summonPlusOneAbilityClick(summonPlusOneAbilityLine)
							}
							className={
					...
					<Row className="hidden-xs">
						<Col xs={12} md={12}>
							<blockquote>
								...
								<p>
									Each type of enhancement has a base cost. The cost
									might then be modified based on which ability is
									being enhanced.
								</p>
								<p>
						...
						<div>
							...
							<Row>
								<Col className="enhancement-col" xs={12} md={12}>
									<Button
										variant="outline-secondary"
										disabled={!this.doubleMultipleTargets()}
										block
										onClick={() => this.multipleTargetClick()}
										className={this.state.multipleTargets && "active"}
									>
							...
							<Row>
								<Col className="enhancement-col" xs={12} md={12}>
									<Button
										variant="outline-secondary"
										block
										onClick={() => this.lostCardClick()}
										className={this.state.lostCard && "active"}
									>
										{this.state.lostCard
											? "Yes (Halve base cost)"
											: "No"}
								...
								<Row>
									<Col className="enhancement-col" xs={12} md={12}>
										<Button
											variant="outline-secondary"
											block
											onClick={() => this.persistentBonusClick()}
											className={
												this.state.persistentBonus && "active"
											}
										>
											{this.state.persistentBonus
